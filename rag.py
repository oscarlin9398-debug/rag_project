...
def retrieve_for_question(q: str, prev_crop: str = None, prev_pest: str = None, is_followup: bool = False) -> dict:
    ...
    if crop and ptype:
        if ptype in ('殺蟲', '殺螨'):
            pest_keywords = ['%蟲%', '%蛾%', '%蝶%', '%薊馬%', '%蚜%', '%蝨%', '%蟎%', '%線蟲%']
        elif ptype == '殺菌':
            pest_keywords = ['%病%', '%菌%', '%腐%', '%疽%', '%疫%', '%萎%']
        elif ptype == '除草':
            pest_keywords = ['%草%']
        else:
            pest_keywords = [f"%{ptype}%"]

        clause = " OR ".join(["病蟲害名稱 LIKE ?"] * len(pest_keywords))
        sql = f"""SELECT 農藥中文普通名稱, 病蟲害名稱, 稀釋倍數, 使用時期, 安全採收期_天, 施藥間隔 
                  FROM pesticides 
                  WHERE 作物名稱=? AND ({clause}) 
                  GROUP BY 農藥中文普通名稱 LIMIT 15"""
        rows = conn.execute(sql, [crop] + pest_keywords).fetchall()
        if rows:
            sql_ctx = f"【{crop} {ptype}劑 登記用藥推薦】\n" + "\n".join(
                f"• {r['農藥中文普通名稱']}（防治{r['病蟲害名稱']}）：稀釋{r['稀釋倍數']}倍，{r['使用時期']}，採收期{r['安全採收期_天']}天"
                for r in rows
            )
            sources.append({"title": f"{crop}{ptype}劑登記用藥", "url": "https://pesticide.aphia.gov.tw", "date": SOURCE_VERSIONS["pesticide"]})
    ...
    if any(k in q for k in QA_KEYWORDS):
        ...
        qa_docs = hybrid_retrieve("qa_knowledge", qa_vs, q_normalized, k=10)
...
class PesticideUpsertRequest(BaseModel):
    crop: str
    pest: str
    name: str
    dosage: Optional[str] = ""
    ...
    timing: Optional[str] = ""
    phi_days: Optional[str] = ""
    interval: Optional[str] = ""
    notes: Optional[str] = ""


@router.post("/api/admin/pesticides/upsert", tags=["管理員"], summary="增量新增或更新農藥登記（免重構全庫）")
def upsert_pesticide(req: PesticideUpsertRequest, username: str = Depends(get_current_user)):
    conn = get_db()
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO pesticides (作物名稱, 病蟲害名稱, 農藥中文普通名稱, 農藥含量, 稀釋倍數, 使用時期, 安全採收期_天, 施藥間隔, 注意事項)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (req.crop, req.pest, req.name, req.dosage, req.dilution, req.timing, req.phi_days, req.interval, req.notes))
    conn.commit()
    new_id = cur.lastrowid
    conn.close()

    # 重新整理記憶體中的 BM25 快取
    if "pesticides" in _bm25_cache:
        del _bm25_cache["pesticides"]
        cfg = _TABLE_CONFIG["pesticides"]
        get_bm25_index("pesticides", cfg["fetch_columns"], cfg["score_columns"])

    return {"message": f"成功增量登記「{req.crop} - {req.name}」", "id": new_id, "status": "BM25 索引已即時同步"}
